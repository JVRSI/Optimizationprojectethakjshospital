...
def runs(size, record_history_of_best_and_worst, cities_matrix, seed=40):
    ...
    return [
        ###################################################################################################
        # Fluid, vary crossover strategy
        ###################################################################################################
        #-----------------------------------------------------------------------------------------------------------
        # 0 Fluid Classic, m:0.8694, c:0.6554, single_break
        GAFactory(
            ga_config = replace(gc,
                random_amount=True,
                crossover_strategy="single_break",
        ...
        #-----------------------------------------------------------------------------------------------------------
        # 1 Fluid Classic, m:0.8694, c:0.6554, single_grid
        GAFactory(
            ga_config = replace(gc,
                random_amount=True,
                crossover_strategy="single_grid",
                probability_of_mutation=0.8694,
                probability_of_crossover=0.6554
            ),
            generator_factory = lambda config, seed: GravityGenerator(rng=np.random.default_rng(seed),config=config,cities_matrix=cities_matrix),
        ...
        #-----------------------------------------------------------------------------------------------------------
        # 2 Fluid Classic, m:0.8694, c:0.6554, grid 10
        GAFactory(
            ga_config = replace(gc,
                random_amount=True,
        ...
        #-----------------------------------------------------------------------------------------------------------
        # 3 Fluid Classic, m:0.8694, c:0.6554, grid 30
        GAFactory(
            ga_config = replace(gc,
                random_amount=True,
                crossover_strategy="grid",
        ...
        ###################################################################################################
        #-----------------------------------------------------------------------------------------------------------
        # 4 Fixed Classic, m:0.8694, c:0.6554, positive_gene_exchange
        GAFactory(
            ga_config = replace(gc,
                probability_of_mutation=0.8694,
                probability_of_crossover=0.6554,
                crossover_strategy="positive_gene_exchange",
            ),
            generator_factory = lambda config, seed: GravityGenerator(rng=np.random.default_rng(seed),config=config,cities_matrix=cities_matrix),
            selector_factory = lambda n_parents, seed: TruncateSelection(n_parents=n_parents,rng=np.random.default_rng(seed)),
            variator_factory = lambda config, seed: ClassicVariation(ga_config=config,rng=np.random.default_rng(seed)),
        ...
        #-----------------------------------------------------------------------------------------------------------
        # 5 Fixed Classic, m:0.8694, c:0.6554, positive_grid 10
        GAFactory(
            ga_config = replace(gc,
                probability_of_mutation=0.8694,
                probability_of_crossover=0.6554,
        ...
        #-----------------------------------------------------------------------------------------------------------
        # 6 Fixed Classic, m:0.8694, c:0.6554, positive_grid 30
        GAFactory(
            ga_config = replace(gc,
                probability_of_mutation=0.8694,
                probability_of_crossover=0.6554,
