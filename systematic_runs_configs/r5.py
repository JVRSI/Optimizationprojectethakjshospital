...
def runs(size, record_history_of_best_and_worst, cities_matrix, seed=40):
    ...
    return [
        #-----------------------------------------------------------------------------------------------------------
        # 0 Try to get the admitted vs cost plot
        GAFactory(
        ...
        #------------------------------------------------------------------------
        # 1 death rate vs admitted tick
        GAFactory(
            ga_config = replace(gc,
                random_amount=True,
                mean_hospital_large=15,
        ...
        ############################################################################################################
        ############################################################################################################
        # 2 Fixing #hospitals, no coverage
        #---------------------------------------------------------------------
        #MC, Basic generator
        GAFactory(
            ga_config = replace(gc,
        ...
        GAFactory(
            ga_config = replace(gc,
                ...
                population_size = 20,
                std_threshold=0.006,
                n_elites = 0, #because n_parents - population size is already a elite
            ),
            generator_factory = lambda config, seed: GravityGenerator(
                rng=np.random.default_rng(seed),
                config=config,
            ...
            variator_factory = lambda config, seed: MicroGAVariation(
                ga_config=config,
                rng=np.random.default_rng(seed)
            ),
            generator_name="GravityGenerator",
